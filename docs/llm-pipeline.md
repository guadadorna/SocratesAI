...
- **[RESUELTO 2026-07-29] El tutor no bajaba la complejidad ante confusión sostenida sobre el mismo punto, y llegó a inventar contenido no presente en el material tratando de resolverla.** Confirmado con el eval-harness (persona `alumno_confuso_ofuscado`, agregada el 2026-07-28, pensada para simular confusión que no se autocorrige rápido): cuando el alumno señalaba la misma confusión puntual dos veces seguidas, el tutor reaccionaba sumando cada vez más capas de explicación (otra limitación, otra conexión) en vez de simplificar — justo lo contrario de lo que recomienda la literatura de tutoring/scaffolding (Rosenshine, VanLehn: ante fracaso repetido con el nivel de andamiaje actual, hay que bajarlo, no subirlo; ver también "scaffolding collapse" en tutores socráticos LLM, arXiv 2607.19371). Un primer intento de fix (prohibir sumar matices) provocó algo peor: al tratar de anclar la explicación con algo concreto, el tutor **inventó ejemplos y después cifras/porcentajes hipotéticos** que no están en el material — evidencia de que la regla de "no inventar" ya existente (repetida con distinta redacción en 3 lugares del prompt: `FUENTE Y ALCANCE`, la sección nueva, y `RESTRICCIONES`) se diluía en vez de reforzarse por estar repetida. Se agregó una sección nueva `SI EL ESTUDIANTE SIGUE SIN ENTENDER UN PUNTO PUNTUAL` (ante la segunda repetición de la misma confusión, simplifica usando solo lo ya dicho en la conversación, y avanza reconociendo el límite si persiste después de un intento) y una regla nueva de máxima prioridad al principio de todo el prompt (`REGLA ABSOLUTA: NUNCA INVENTES`, cubre explícitamente números/cifras hipotéticas además de ejemplos/escenarios) que reemplaza las dos menciones diluidas por una referencia corta — sin tocar `FUENTE Y ALCANCE` (la escribió Guada, se prefirió reforzar en vez de reescribir). Validado con replays puntuales sobre el mismo tramo real de conversación (3/3 sin inventar nada, contra 2/3 y luego 0/2 en los intentos previos) y con el harness completo.
...
5. **Nuevo aprendizaje 2026-07-29**: repetir la misma regla en varios lugares del prompt (ej. "no inventar" aparecía con distinta redacción en 3 secciones de `getTutorPrompt`) no la refuerza — puede diluirla, y el modelo la violó igual. `getTutorPrompt` creció a ~250 líneas por acumulación aditiva de fixes (literalismo, repetición/hartazgo, enumeraciones, confusión sostenida); queda pendiente una segunda pasada de condensación (fusionar la redundancia de "no exigir las mismas palabras del material", que aparece 4 veces, y compactar la prosa de las 6 sub-reglas de reacción) antes de seguir agregando secciones nuevas al tutor.
