...
- [ ] **Foco de la proxima sesion: FEEDBACK** (el tema del tutor ya se dio por cerrado por ahora). Problema confirmado el 2026-07-29 con corrida de chequeo final (`chequeo-final-tutor-y-feedback`, 5 personas x 4 repeticiones de evaluacion = 20 evaluaciones): solo 2 de 20 tuvieron headings/negrita detectables pese a que `getCombinedEvaluationPrompt` lo pide explicitamente — el resto devolvio el contenido completo y correcto pero en prosa corrida. No es un problema del harness/detector: es que el modelo ignora sistematicamente el formato pedido en el prompt real que ve el alumno y la profesora. Ver `docs/llm-pipeline.md` seccion 2 para el detalle tecnico (parseo, `stripPreamble`, delimitador)
