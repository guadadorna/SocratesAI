...
- [ ] **PRIORIDAD**: reescribir la seccion "COMO REACCIONAR A LAS RESPUESTAS" de `getTutorPrompt` (`src/lib/prompts.ts`) — hoy el tutor evalua las respuestas comparandolas contra frases especificas y literales del material en vez de juzgar si el concepto subyacente esta entendido. Confirmado con una sesion real (2026-07-27, material de Metodo de Control Sintetico): respuestas correctas o mas precisas que el material ("series de tiempo", "trasladable a otras unidades") fueron tratadas como incorrectas por no matchear la frase exacta del PDF. Esto confunde mas de lo que ayuda y va en contra del objetivo socratico. No lo arregla la temperatura ni el modelo, es un problema de instruccion del prompt
